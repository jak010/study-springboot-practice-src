...
|---------|---------|---------------|------|-------|---------------------------------------------------------------|---------------------|
| MEM_01  | 회원가입    | 회원 가입         | A    | ⬜ 미구현 | `registerMember()`                                            | -                   |
| MEM_02  | 회원조회    | 회원 정보 조회 (단건) | A    | check | `getMemberById(Long memberId)`                                | -                   |
| MEM_03  | 회원조회    | 회원 정보 조회 (목록) | A    | check | `getAllMembers()`                                             | 페이징 가능              |
| MEM_04  | 회원수정    | 회원 상태 변경      | A    | ⬜ 미구현 | `updateMemberStatus(Long memberId, Status status)`            | -                   |
| MEM_05  | 회원가입/검증 | 이메일 중복 체크     | A    | ⬜ 미구현 | `isEmailDuplicate(String email)`                              | boolean 반환          |
| MEM_06  | 회원가입/검증 | 닉네임 중복 체크     | A    | ⬜ 미구현 | `isNicknameDuplicate(String nickname)`                        | boolean 반환          |
